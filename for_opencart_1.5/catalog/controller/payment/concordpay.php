...
class ControllerPaymentConcordpay extends Controller
{
    ...
    {
        $this->language->load('payment/concordpay');
        $this->load->model('checkout/order');

        $fields = $this->generateFields();

        $this->data['fields'] = $fields;
        $this->data['action'] = Concordpay::URL;
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/concordpay.tpl')) {
            $this->template = $this->config->get('config_template') . '/template/payment/concordpay.tpl';
        } else {
            $this->template = 'default/template/payment/concordpay.tpl';
        }
    ...
    public function generateFields()
    {
        ...
        $secret_key = $this->config->get('concordpay_secretkey');
        $concordpay->setSecretKey($secret_key);

        $this->load->model('checkout/order');
        $order = $this->model_checkout_order->getOrder($this->session->data['order_id']);

        $approve_url  = $this->config->get('concordpay_approve_url');
        $decline_url  = $this->config->get('concordpay_decline_url');
        $cancel_url   = $this->config->get('concordpay_cancel_url');
        $callback_url = $this->config->get('concordpay_callback_url');
        $language     = $this->config->get('concordpay_language');
        ...
        if ($payment_info === true) {
            ...
            if ($data['type'] === ConcordPay::RESPONSE_TYPE_PAYMENT) {
                $this->model_checkout_order->update(
                    $order_id,
                    $this->config->get('concordpay_order_status_id'),
                    'ConcordPay'
                );
            } elseif ($data['type'] === ConcordPay::RESPONSE_TYPE_REVERSE) {
                $this->model_checkout_order->update(
                    $order_id,
                    $this->config->get('concordpay_order_reverse_status_id'),
                    'ConcordPay'
                );
            }
        } else {
