...
namespace Opencart\Catalog\Controller\Extension\OcPaymentConcordpay\Payment;
...
class Concordpay extends \Opencart\System\Engine\Controller
{
    ...
    {
        $this->language->load('extension/oc_payment_concordpay/payment/concordpay');
        $this->load->model('checkout/order');

        $fields = $this->generateFields();

        $data['action'] = ConcordpayApi::URL;
        ...
        return $this->load->view('extension/oc_payment_concordpay/payment/concordpay', $data);
    }
    ...
    {
        $concordpay = new ConcordpayApi();
        $secret_key = $this->config->get('payment_concordpay_secretkey');
        $concordpay->setSecretKey($secret_key);

        $this->load->model('checkout/order');
        $order = $this->model_checkout_order->getOrder($this->session->data['order_id']);

        $approve_url  = $this->config->get('payment_concordpay_approve_url');
        $decline_url  = $this->config->get('payment_concordpay_decline_url');
        $cancel_url   = $this->config->get('payment_concordpay_cancel_url');
        $callback_url = $this->config->get('payment_concordpay_callback_url');
        $language     = $this->config->get('payment_concordpay_language');
    ...
    public function callback()
    {
        ...
        $concordpay->setSecretKey($key);

        $payment_info = $concordpay->isPaymentValid($data);
        ...
        if ($payment_info === true) {
            ...
            if ($data['type'] === ConcordpayApi::RESPONSE_TYPE_PAYMENT) {
                $this->model_checkout_order->addHistory(
                    $order_id,
                    (int) $this->config->get('payment_concordpay_order_status'),
                    'ConcordPay',
                    true
                );
                ...
                $this->model_checkout_order->addHistory(
                    $order_id,
                    (int) $this->config->get('payment_concordpay_order_reverse_status'),
                    'ConcordPay',
                    true
                );
            }
